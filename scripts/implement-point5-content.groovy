...
addCta(casesArea, 'Welches Muster erkennst du wieder?', '<p>In der Clarity Session betrachten wir eine konkrete Situation vertraulich und trennen Symptom, mögliche Ursache und nächsten sinnvollen Test.</p>', '/clarity-session', 'Clarity Session ansehen')
...
checkHtml << '<p class="chos-check__privacy"><strong>Datensparsam:</strong> Die Auswertung findet ausschließlich in diesem Browser statt. Dabei werden keine Antworten übertragen oder gespeichert und keine Cookies gesetzt.</p>'
checkHtml << '<form data-chos-form><p>Bewerte jedes Signal aus Sicht deiner aktuellen Organisation. Der Check dauert etwa acht bis zehn Minuten.</p>'
...
checkHtml << '<section class="chos-check__result" data-chos-result hidden tabindex="-1" aria-live="polite"><p class="mvp-meta">Deine Einordnung</p><div data-chos-summary></div><ul class="chos-check__result-list" data-chos-dimensions></ul><a class="button" data-chos-result-cta href="/clarity-session">Ergebnis persönlich einordnen</a></section>'
...
addHero(checkArea, 'Kostenloser Selbstcheck', 'Wo lohnt sich genaueres Hinsehen?', '20 Fragen geben eine erste Orientierung zu fünf Spannungsfeldern deiner Produktorganisation – ohne Registrierung und ohne Übertragung deiner Antworten.')
...
addText(checkArea, 'Was du mit dem Ergebnis tun kannst', '<ol><li>Wähle ein konkretes Beispiel aus der am stärksten ausgeprägten Dimension.</li><li>Trenne beobachtbares Verhalten von deiner ersten Erklärung.</li><li>Formuliere mindestens eine alternative Hypothese.</li><li>Prüfe einen kleinen Eingriff, dessen Wirkung beobachtbar ist.</li></ol><p>Für eine belastbare Organisationsdiagnose braucht es zusätzlichen Kontext, Gespräche und reale Entscheidungsverläufe.</p>', true)

Node chosPage = website.getNode('/chos')
chosPage.setProperty('dateModified', '2026-09-02')
